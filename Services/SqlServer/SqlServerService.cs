...
{
    /// <summary>
    /// Service for handling SQL Server database operations
    /// </summary>
...
from [MediaStudio].[Posts].[UNFIPostCampaignStore] pcs with (nolock)";
        ...
        public async Task<bool> TestMediaStudioConnectionAsync()
        {
            try
            {
                using (var connection = new SqlConnection(_connectionString))
        ...
        public async Task<List<Campaign>> GetCampaignsAsync()
        {
          ...
          FROM [dbo].[Campaign] with (nolock)";
            ...
            try
            {
                ...
                {
                    await connection.OpenAsync();
                    Console.WriteLine("Connected to AdStudioUnfi database successfully");
                        ...
                        using (var reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                var campaign = new Campaign
                                {
                                    ...
                                    BrandId = reader.GetInt32(1), // BrandId
                                    Name = reader.IsDBNull(2) ? string.Empty : reader.GetString(2), // Name
                                    Description = reader.IsDBNull(3) ? string.Empty : reader.GetString(3), // Description
                                    StartDate = reader.IsDBNull(4) ? string.Empty : reader.GetString(4), // StartDate
                                    EndDate = reader.IsDBNull(5) ? string.Empty : reader.GetString(5) // EndDate
                                };
            ...
            {
                Console.WriteLine($"SQL Error retrieving campaigns: {ex.Message}");
                throw;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Saves campaigns to JSON file
        /// </summary>
        /// <param name="campaigns">List of campaigns to save</param>
        /// <param name="filePath">Path where to save the JSON file</param>
        public async Task SaveCampaignsToJsonAsync(List<Campaign> campaigns, string filePath)
        {
            try
            {
                var json = JsonConvert.SerializeObject(campaigns, Formatting.Indented);
                await Task.Run(() => File.WriteAllText(filePath, json));
                Console.WriteLine($"Campaigns saved to JSON file: {filePath}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error saving campaigns to JSON: {ex.Message}");
                throw;
            }
        }

        /// <summary>
        /// Saves campaigns to CSV file
        /// </summary>
        /// <param name="campaigns">List of campaigns to save</param>
        /// <param name="filePath">Path where to save the CSV file</param>
        public async Task SaveCampaignsToCsvAsync(List<Campaign> campaigns, string filePath)
        {
            try
            {
                using (var writer = new StreamWriter(filePath))
                {
                    // Write CSV header
                    await writer.WriteLineAsync("Id,BrandId,Name,Description,StartDate,EndDate");

                    // Write data rows
                    foreach (var campaign in campaigns)
                    {
                        var line = $"{campaign.Id},{campaign.BrandId},\"{EscapeCsvField(campaign.Name)}\",\"{EscapeCsvField(campaign.Description)}\",\"{campaign.StartDate}\",\"{campaign.EndDate}\"";
                        await writer.WriteLineAsync(line);
                    }
                }

                Console.WriteLine($"Campaigns saved to CSV file: {filePath}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error saving campaigns to CSV: {ex.Message}");
                throw;
            }
        }
